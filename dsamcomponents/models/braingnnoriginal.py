...
class Network(torch.nn.Module):
    def __init__(self, indim, ratio, nclass, n_hidden_layers, n_fc_layers, k=7, dropout=0.2, R=100):
        ...
        self.dim1 =n_hidden_layers[0] #64
        self.dim2 = n_hidden_layers[1]#64
        self.dim3 = 512
        self.dim4 = 256
        self.dim5 = 8
        self.k = k
        ...
        self.conv1 = MyGINConv(self.indim, self.dim1, self.n1, normalize=False)
        ...
        self.conv2 = MyGINConv(self.dim1, self.dim2, self.n2, normalize=False)
    ...
    def forward(self, x, edge_index, batch, edge_attr, pos):
        ...
        return x, [self.pool1,self.pool2], [torch.sigmoid(score1).view(x.size(0),-1), torch.sigmoid(score2).view(x.size(0),-1)], edge_attr
        # return x, [self.pool1, self.pool2], [score1, score2], edge_attr
