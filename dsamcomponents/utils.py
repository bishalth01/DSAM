...
class SweepType(str, Enum):
    ...
    BRAIN_GNN = 'brain_gnn'
    GIN = 'gin'
    META_NODE_POOL = 'node_meta_pool'
    GRAPH_MULTISET_TRANSFORMER = 'graph_transformer'
    STAGIN = 'stagin'
...
class EarlyStopping:
    ...
    def save_model_and_metrics(self, val_metrics, model, label_scaler):
        ...
        # torch.save(model.state_dict(), os.path.join("/data/users3/bthapaliya/BrainNetworkTransformer-main/saved_models", self.model_saving_name))
        torch.save(model.state_dict(), os.path.join('/data/users3/bthapaliya/BrainNetworkTransformer-main/saved_models',self.model_saving_name))
...
class MyDataLoader(DataLoader):
    def __init__(self, dataset, batch_size, shuffle=True, **kwargs):
        super(MyDataLoader, self).__init__(dataset, batch_size=batch_size, shuffle=shuffle, **kwargs)

    def __iter__(self):
        return MyDataLoaderIter(super().__iter__())

class MyDataset(Dataset):
    def __init__(self, data):
        self.x = data.x
        self.y = data.y
        self.tempdata = deepcopy(data)
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        x = self.data.x[:, idx, :]  # Select the feature data for the corresponding target index
        y= self.data.y[idx]
        self.tempdata.x= x
        self.tempdata.y = y
        return self.tempdata.x, self.tempdata.y

class MyDataLoaderIter:
...
def change_w_config_(w_config):
    ...
    w_config['split_to_test'] = w_config['fold_num']
    ...
    w_config['ts_spit_num'] = int(4800 / w_config['time_length'])

    w_config['param_gat_heads'] = 0
    if w_config['sweep_type'] == SweepType.GAT:
        w_config['param_gat_heads'] = w_config.gat_heads

    w_config['lr_scheduler'] = LRScheduler(w_config['lr_scheduler'])
    w_config['optimiser'] = Optimiser(w_config['optimiser'])
...
class WarmUpLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        """
        tol = 1e-8 if (self.total_iters == 0.) else 0.
        return [base_lr * self.last_epoch / (self.total_iters + tol) for base_lr in self.base_lrs]
