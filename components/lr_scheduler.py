...
class LRScheduler:
    def __init__(self, cfg: DictConfig,  optimizer_cfg: DictConfig):
        ...
        assert self.lr_config.mode in [
            'step', 'poly', 'cos', 'linear', 'decay','onecycle']
    ...
    def update(self, optimizer: torch.optim.Optimizer, step: int):
        ...
        target_lr = base_lr / (10 ** lr_config.target_lr)
        ...
        if step < warm_up_steps:
            ...
            if lr_mode == 'step':
                ...
                lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=lr_config.base_lr, steps_per_epoch=self.training_config.steps_per_epoch, epochs=200)
                self.lr = lr_scheduler.get_last_lr()[0]
        ...
        # assert 0 <= step <= total_steps
        # if step < warm_up_steps:
        #     current_ratio = -100 / warm_up_steps
        #     self.lr = warm_up_from + (base_lr - warm_up_from) * current_ratio
        # else:
        #     current_ratio = (step - warm_up_steps) / \
        #         (total_steps - warm_up_steps)
