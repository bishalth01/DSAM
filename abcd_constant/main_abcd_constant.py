...
from abcd_constant.model_abcd_constant import SpatioTemporalModel
...
def create_fold_generator(dataset: BrainDataset, run_cfg: Dict[str, Any], num_splits: int):
    if run_cfg['dataset_type'] == DatasetType.HCP:
        ...
        # merged_labels = merge_y_and_others(torch.cat([data.y for data in dataset], dim=0),
        #                                    torch.cat([data.index for data in dataset], dim=0))
        # merged_labels = np.array(int(dataset.data.y.cpu().detach().numpy()))
        merged_labels = np.array(dataset.data.y.cpu().detach().numpy()).astype(int)
        skf_generator = skf.split(np.zeros((len(dataset), 1)),
                                  merged_labels,
                                  groups=np.array([data.index.numpy().item() for data in dataset]))
...
if __name__ == '__main__':
    ...
    # DATASET
    #dataset = generate_dataset_atlas(sourcedir="/data/qneuromark/Data/HCP/Data_BIDS/Preprocess_Data",k_fold=None)
    #torch.save(dataset,'abcd_hcp_schaefer.pt')
    dataset = generate_dataset(run_cfg)

    #dataset =  DatasetHCPRest(sourcedir="/data/qneuromark/Data/HCP/Data_BIDS/Preprocess_Data",k_fold=None)
    #dataset = generate_dataset_atlas(sourcedir="/data/qneuromark/Data/HCP/Data_BIDS/Preprocess_Data",k_fold=None)
    #torch.save(dataset,'abcd_hcp_schaefer.pt')
    
    #newdataset = torch.load('abcd_hcp_schaefer.pt')

    #dataset = CustomDatasetHCP(torch.tensor(newdataset['data']), torch.tensor(newdataset['labels']))
