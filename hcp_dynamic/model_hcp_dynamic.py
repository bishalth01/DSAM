...
class SpatioTemporalModel(nn.Module):
    def __init__(self, run_cfg: Dict[str, Any],
        ...
        if self.sweep_type == SweepType.GCN:
            ...
            #self.meta_layer = LayerGIN(input_dim=self.NODE_EMBED_SIZE, hidden_dim=32, output_dim= self.NODE_EMBED_SIZE)
            #self.meta_layer = GraphCNN(5,2,self.NODE_EMBED_SIZE,64,2,0.5,False,"sum","sum", "cuda")
            self.meta_layer = GINNet(run_cfg['dataset_indegree']).to('cuda')
            ...
            parser.add_argument('--conv', default='GCN', type=str,choices=['GCN', 'GIN'],help='message-passing function type')
            parser.add_argument('--indim', type=int, default=32, help='feature dim')
            parser.add_argument('--num-convs', default=2, type=int)
            parser.add_argument('--mab-conv', default='GCN', type=str, choices=['GCN', 'GIN'],help='Multi-head Attention Block, GNN type')
            parser.add_argument('--num-hidden', type=int, default=64, help='hidden size')
            parser.add_argument('--num-heads', type=int, default=2, help='attention head size')
            parser.add_argument("--ln", action='store_true')
            parser.add_argument("--lr-schedule", action='store_true')
            parser.add_argument("--cluster", action='store_true')
            parser.add_argument("--normalize", action='store_true')
            parser.add_argument("--model-string", type=str, default='GMPool_G-SelfAtt-GMPool_I')
            parser.add_argument("--grad-norm", type=float, default=1.0)
            parser.add_argument("--dropout", type=float, default=0.3)
            parser.add_argument('--pooling-ratio', type=float, default=0.20, help='pooling ratio')
            parser.add_argument('--num-epochs', default=500, type=int, help='train epochs number')
            parser.add_argument("--gpu", type=int, default=1)
            parser.add_argument('--patience', type=int, default=50, help='patience for earlystopping')
            parser.add_argument('--num-features', type=int, default=100, help='feature dim')
            parser.add_argument('--num-classes', type=int, default=1, help='number of classes')
            parser.add_argument('--avg_num_nodes', type=int, default=100, help='average number of nodes - for dynamic graphs')

            
            opt = parser.parse_args()


            # self.meta_layer = GraphMultisetTransformer(num_node_features=self.NODE_EMBED_SIZE, num_edge_features=1,
            #                                activation=self.activation, run_cfg=run_cfg, pool_ratio=run_cfg['pnapoolratio'])
        ...
        if self.conv_strategy == ConvStrategy.TCN_ENTIRE:
            ...
            if run_cfg['tcn_hidden_units'] == 8:
                ...
                #print("Shape before lin temporal is " + str(self.size_before_lin_temporal))
    ...
    def run_tcn_gnn_model(self, data, x, edge_index, edge_attr):
        ...
        #self.temporal_conv = temporal_model
        ...
        if self.conv_strategy != ConvStrategy.NONE:
            ...
            # x = x.view(x.size()[0], self.size_before_lin_temporal) #1185*4 

            # original_number_timepoints = int(self.size_before_lin_temporal/self.number_hidden_units)

            # batch_size = int(data.batch.shape[0]/self.num_nodes)

            # x= x.reshape(int(data.batch.shape[0]/self.num_nodes), self.num_nodes, -1)  #(100, 400, 1172)
...
###########################TO RECALCULATE EDGES= DYNAMIC WAY #####################################

            normalized_matrix_masked_output_combined = masked_output_combined
            
            batch_size, num_nodes, _ = corr.shape
            # Initialize an empty tensor to store the correlations
            finalcorrelations = torch.empty((batch_size, num_nodes, num_nodes)).to(device)
            # Loop through each data point in the batch
            for i in range(batch_size):
                data_point = normalized_matrix_masked_output_combined[i]  # Get the data point (400, 373)
                
                # Transpose the data point to (373, 400) and compute the correlation
                corr_matrix = torch.abs(torch.corrcoef(data_point))
                
                # Store the correlation matrix for the current data point
                finalcorrelations[i] = corr_matrix
                

            if self.threshold == 100:
                # Set diagonal elements to zero
                _, num_nodes, _ = finalcorrelations.shape
                computed_final_edges = finalcorrelations.flatten().unsqueeze(dim=1)
                edge_attr[edge_attr != 0] = computed_final_edges[edge_attr != 0]

            else:
               
                 # Calculate batch indices for all edges
                edges_in_each_batch = edge_attr.shape[0]/batch_size
                # edge_batch_indices = torch.arange(edge_index.size(1)) // edges_in_each_batch
                edge_batch_indices = torch.div(torch.arange(edge_index.size(1)), edges_in_each_batch, rounding_mode='trunc')
                # Calculate node numbers for source and target nodes for all edges
                node_numbers_source = edge_index[0] % self.num_nodes
                node_numbers_target = edge_index[1] % self.num_nodes
                # Initialize an empty tensor to store correlation values
                correlation_values = torch.empty(edge_index.size(1), dtype=edge_attr.dtype, device=edge_attr.device)
                # Process each batch separately
                for batch_idx in range(batch_size):
                    # Get indices for the current batch
                    batch_mask = (edge_batch_indices == batch_idx)
                    batch_indices = torch.nonzero(batch_mask).squeeze()
                    if len(batch_indices) > 0:
                        # Get the correlation matrix for the current batch
                        correlation_matrix_batch = finalcorrelations[batch_idx]
                        # Get the source and target nodes for the current batch
                        batch_node_numbers_source = node_numbers_source[batch_indices]
                        batch_node_numbers_target = node_numbers_target[batch_indices]
                        # Extract correlation values for the current batch
                        batch_correlation_values = correlation_matrix_batch[batch_node_numbers_source, batch_node_numbers_target]
                        # Assign correlation values to the appropriate positions in the result tensor
                        correlation_values[batch_indices] = batch_correlation_values
                # Assign correlation values to edge_attr
                edge_attr = correlation_values.unsqueeze(dim=1).detach()
        ...
        if self.sweep_type in [SweepType.GAT, SweepType.GCN]:
        ...
        elif self.sweep_type == SweepType.STAGIN:
            ...
            a = self._collate_adjacency(a, sparsity)
    ...
    def forward(self, data, v=None, a=None, t=None, sampling_endpoints=None):
        ...
        # #Create a list of processes for parallel execution
        # processes = []
        # output_queues = []

        # # Create an output queue for each GNN model
        # for _ in range(1):
        #     output_queue = Queue()
        #     output_queues.append(output_queue)

        # # Start the processes and pass the output queue to each process
        # processes.append(Process(target=self.run_tcn_gnn_model, args=(data,x,edge_index, edge_attr, self.temporal_conv1, self.meta_layer, output_queues[0])))
        # #processes.append(Process(target=self.run_tcn_gnn_model, args=(data,x,edge_index, edge_attr, self.temporal_conv2, self.meta_layer, output_queues[1])))
        # #processes.append(Process(target=self.run_tcn_gnn_model, args=(data,x,edge_index, edge_attr, self.temporal_conv3, self.meta_layer, output_queues[2])))
        # #processes.append(Process(target=self.run_tcn_gnn_model, args=(data,x,edge_index, edge_attr, self.temporal_conv4, self.meta_layer, output_queues[3])))
        ...
        # # Retrieve the outputs from the output queues
        # gnn1_output = output_queues[0].get()
        # gnn2_output = output_queues[1].get()
        # gnn3_output = output_queues[2].get()
        # gnn4_output = output_queues[3].get()

        #gnn_outputs = [gnn1_output, gnn2_output, gnn3_output, gnn4_output]

        output_1, allpools_1, scores_1 = self.run_tcn_gnn_model(data, x, edge_index, edge_attr)
        #torch.cuda.empty_cache()
        #output_2, allpools_2, scores_2 = self.run_tcn_gnn_model(data, x, edge_index, edge_attr)
        #torch.cuda.empty_cache()
        # output_3 = self.run_tcn_gnn_model(data, x, edge_index, edge_attr, self.temporal_conv3, self.meta_layer)
        # torch.cuda.empty_cache()
        # output_4 = self.run_tcn_gnn_model(data, x, edge_index, edge_attr, self.temporal_conv4, self.meta_layer)
        # torch.cuda.empty_cache()
        # torch.cuda.empty_cache()


        # Compute KL Divergence (KLD) loss between pairs of output distributions
        #kl_div_loss_12 = F.kl_div(output_1, output_2, reduction='batchmean')
        # kl_div_loss_13 = F.kl_div(output_1, output_3, reduction='batchmean')
        # kl_div_loss_14 = F.kl_div(output_1, output_4, reduction='batchmean')
        # kl_div_loss_23 = F.kl_div(output_2, output_3, reduction='batchmean')
        # kl_div_loss_24 = F.kl_div(output_2, output_4, reduction='batchmean')
        # kl_div_loss_34 = F.kl_div(output_3, output_4, reduction='batchmean')

        # total_kl_div_loss = kl_div_loss_12 + kl_div_loss_13 + kl_div_loss_14 + kl_div_loss_23 + kl_div_loss_24 + kl_div_loss_34


        # x = torch.concat([output_1, output_2, output_3, output_4], dim=1)

        
        #total_kl_div_loss = kl_div_loss_12 


        # #x = torch.concat([output_1, output_2], dim=1)
        # x = output_1
        # #x = nn.ReLU()(x)
        ...
        #     if self.sweep_type == SweepType.BRAIN_GNN:
        #          return torch.sigmoid(x), allpools_1, scores_1
        #     #  else:
