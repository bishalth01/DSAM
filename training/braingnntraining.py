...
class BrainGNNTrain:
    ...
    def train_per_epoch(self, optimizer, lr_scheduler):
        ...
        for data in self.train_dataloader:
            ...
            # node_feature = node_feature.reshape(node_feature.shape[0] * node_feature.shape[1], node_feature.shape[2])
            # node_feature= node_feature.reshape(node_feature.shape[0], node_feature.shape[1], time_series.shape[-1])
            ...
            predict, _, _ = self.model(data)

            loss = self.loss_fn(predict, label.squeeze())
